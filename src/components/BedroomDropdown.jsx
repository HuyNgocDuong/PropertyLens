...
const BedroomDropdown = () => {
  const { bedroom, setBedroom, bedrooms } = useContext(HouseContext);
  ...
  return (
    <FormControl fullWidth>
      <Select
        ...
        renderValue={(selected) => (
          <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
            <RiHotelBedLine style={{ fontSize: 24, marginRight: 18, color: '#6366f1' }} />
            <Box sx={{ flexGrow: 1 }}>
      ...
      >
        {Array.isArray(bedrooms) && bedrooms.length > 0 ? (
          ...
          <MenuItem disabled>No bedrooms available</MenuItem>
        )}
      </Select>
    </FormControl>
  );
